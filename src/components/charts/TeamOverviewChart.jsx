...
const TeamOverviewChart = ({ players, assessmentResults, assessmentTypes, goals, findGoal, isGoalMet }) => {
  // Calculate goal achievement rates by category
  const categoryStats = assessmentTypes.reduce((acc, assessment) => {
  ...
  const totalEntered = Object.values(assessmentResults).filter(result => result && result !== '').length;
    ...
    datasets: [
      {
        ...
        data: Object.values(categoryStats).map(stat => 
          stat.total - stat.notEntered > 0 ? ((stat.met / (stat.total - stat.notEntered)) * 100).toFixed(1) : 0
        ),
