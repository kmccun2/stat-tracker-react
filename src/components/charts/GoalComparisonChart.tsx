...
interface GoalComparisonChartProps {
  ...
  findGoal: (player: Player, assessmentType: string) => GoalInfo | null;
}

const GoalComparisonChart: React.FC<GoalComparisonChartProps> = ({
  player,
  assessmentResults,
  assessmentTypes,
  findGoal,
  ...
  const playerData: PlayerDataItem[] = assessmentTypes
    .map((assessment) => {
      const result =
      ...
      if (goalInfo.minGoal && goalInfo.maxGoal) {
        goalValue =
          (parseFloat(goalInfo.minGoal) + parseFloat(goalInfo.maxGoal)) / 2;
      ...
      return {
        ...
        goalType,
        unit: goalInfo.unit || "",
        lowIsGood: goalInfo.lowIsGood || false,
      };
    })
    .filter((item): item is PlayerDataItem => item !== null);
