...
import { Container, Row, Col, Card, Table, Button, Badge, ButtonGroup, ToggleButton, Alert } from 'react-bootstrap';
...
const MetricsPage = () => {
  ...
  const metricsByCategory = assessmentTypes.reduce((acc, metric) => {
  ...
  const handleUpdateMetric = (updatedMetric) => {
  ...
  const handleDeleteMetric = async (metric) => {
    ...
    if (!window.confirm(`Are you sure you want to delete "${metric.assessment_type || metric.AssessmentType}"? This action cannot be undone.`)) {
      return;
    }
      ...
      {Object.entries(metricsByCategory).map(([category, metrics]) => (
        <div key={category} className="mb-4">
          <h4 className="text-primary mb-3">
            <Badge bg="primary" className="me-2">{metrics.length}</Badge>
            {category}
          </h4>
          <Row className="g-3">
            {metrics.map(metric => (
              <Col key={metric.id || `${metric.AssessmentType}-${metric.Category}`} xs={12} sm={6} md={4} lg={3}>
  ...
  return (
    <Container>
      ...
      <AddMetricModal
        ...
        existingCategories={[...new Set(assessmentTypes.map(m => m.category || m.Category))].filter(Boolean)}
      />
