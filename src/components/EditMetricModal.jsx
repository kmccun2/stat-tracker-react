...
const EditMetricModal = ({ 
  ...
  useEffect(() => {
    if (metric && show) {
      setFormData({
        ...
        assessment_type_sort: metric.assessment_type_sort || metric.AssessmentTypeSort || '',
        category_sort: metric.category_sort || metric['Category Sort'] || ''
      });
    }
  }, [metric, show]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await apiService.updateAssessmentType(metric.id, submitData);
      if (!response.success) {
